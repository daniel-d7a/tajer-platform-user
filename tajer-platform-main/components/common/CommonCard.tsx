...
export default function ProductCard({
        ...
        <Card className="overflow-hidden flex flex-col h-full rounded-2xl duration-300 transition-transform border-2 hover:border-primary/30">
            ...
            {productDetails.discountAmount > 0 && (
              <Badge className="absolute top-2 right-2 bg-primary z-10">
                {productDetails.discountType === 'percentage' 
                  ? `${productDetails.discountAmount}% ${t('offer')}` 
                  : `${productDetails.discountAmount} ${tc('coins')} ${t('offer')}`}
              </Badge> 
            )}
            <div 
          ...
          <CardContent className="p-4 flex-grow">
            ...
            <div className="flex items-baseline mt-2">
              {productDetails.unitType === "pack_only" || productDetails.unitType === "piece_or_pack" ? (
                <div className="flex gap-2 flex-col w-full">
                      ...
                      {productDetails.discountAmount > 0 ? (
                        <div className="flex gap-2">
                          <span className="text-lg font-bold text-primary">
                            {calculateDiscountedPrice(productDetails, false).toFixed(2)} {tc('coins')}
                          </span>
                          <span className="line-through text-muted-foreground text-sm">
                            {productDetails.piecePrice.toFixed(2)} {tc('coins')}
                          </span>
                        </div>
                      ) : (
                        <span className="text-lg font-bold text-primary">
                  ...
                  <div>
                    <span className="text-md w-[100%] mr-2 font-medium">
                      {t('PackPrice')}: {calculateDiscountedPrice( productDetails, true).toFixed(2)} {tc('coins')}
                      {productDetails.discountAmount > 0 && (
                        <span className="line-through text-muted-foreground text-sm ml-2">
                          {productDetails.packPrice.toFixed(2)} {tc('coins')}
                        </span>
                      )}
                    </span>
                  </div>
                  <div className="flex flex-col gap-2">
                    <span className="text-xs text-muted-foreground">
                      {t('piecesPerPack')}: {productDetails.piecesPerPack} / {language === 'en' ? "pieces" : "قطع في الحزمه"}
                    </span>
                  </div>
                </div>
              ) : (
                <div className="flex items-center gap-2 w-full">
                  {productDetails.discountAmount > 0 ? (
                    <div className="flex gap-2 w-full">
                      <span className="text-lg font-bold text-primary">
                        {calculateDiscountedPrice(productDetails).toFixed(2)} {tc('coins')}
                      </span>
                      <span className="line-through text-muted-foreground text-sm">
                        {productDetails.piecePrice.toFixed(2)} {tc('coins')}
                      </span>
  ...
  createPortal(
    <ImageUpScale 
      src={productDetails.imageUrl || "/placeholder.svg"}
      alt={language === "en" ? productDetails.name : productDetails.name_ar}
