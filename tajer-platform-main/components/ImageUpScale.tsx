...
export default function ImageUpScale({ src, alt, onClose }: ImageUpScaleProps) {
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    const newScale = Math.max(0.5, Math.min(scale + delta * 0.1, 5));
    ...
    if (imageContainerRef.current) {
      ...
      setPosition(prev => ({
        x: prev.x - (mouseX - rect.width / 2) * (scaleChange / scale),
        y: prev.y - (mouseY - rect.height / 2) * (scaleChange / scale)
      }));
    }
  ...
  className="relative w-[80vw] h-[80vh] overflow-hidden flex flex-col"
      ...
      >
        ...
        >
          ...
          <div 
          className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex items-center gap-2 bg-black/50 backdrop-blur-sm rounded-xl p-1 border border-white/20">
            <button 
