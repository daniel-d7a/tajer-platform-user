...
export default function Page() {
  ...
  const fetchData = async (currentPage: number = page, limit: number = itemsPerPage) => {
    setLoading(true);
    try {
      const response = await fetch(
        `https://tajer-backend.tajerplatform.workers.dev/api/orders/orders/user?limit=${limit}&page=${currentPage}&status=${status}&from=&to=`,
        { credentials: "include" }
      );
      ...
      {!loading && orders.length > 0 && (
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="text-sm text-muted-foreground">
            {to('showing')} {(page - 1) * itemsPerPage + 1} - {Math.min(page * itemsPerPage, meta.total)} {to('of')} {meta.total} {to('orders')}
          </div>
