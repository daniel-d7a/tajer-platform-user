...
import { Search, Clock, Trash2, Tag } from "lucide-react";
...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("recentCategorySearches");
    if (stored) {
      try {
        setRecentSearches(JSON.parse(stored));
      } catch (error) {
        console.error("Error parsing recent category searches:", error);
      }
    }
  }, []);
  ...
  const fetchSuggestions = useCallback(async (query: string) => {
    ...
    try {
      const response = await fetch(
        `https://tajer-backend.tajerplatform.workers.dev/api/public/categories?limit=5&page=1&search=${encodeURIComponent(
          query
        )}`
      );
      ...
      if (response.ok) {
        ...
        if (data.data && data.data.length > 0) {
          const categorySuggestions: Suggestion[] = data.data.map(
            (category: Category) => ({
              id: category.id,
              name: category.name,
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    localStorage.setItem("recentCategorySearches", JSON.stringify(updated));
    setShowSuggestions(false);
    setSuggestions([]);
    setActiveSuggestionIndex(-1);
