...
import { Search, Clock, Trash2, Package } from "lucide-react";
    ...
    async (query: string) => {
      ...
      try {
        const response = await fetch(
          `https://tajer-backend.tajerplatform.workers.dev/api/public/products?categoryId=${id}&factoryId=&search=${encodeURIComponent(
            query
          )}&page=1&limit=5`
        );
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(
        `https://tajer-backend.tajerplatform.workers.dev/api/public/categories/${id}`
      );
