...
export default function Page() {
  ...
  const fetchProductById = async () => {
    try {
      const res = await fetch(
        `https://tajer-platform-api.eyadabdou862.workers.dev/api/public/products?categoryId=${categoryIds}&factoryId=&search=&page=&limit=`
      );
  ...
  const fetchOffer = async () => {
    ...
    try {
      const res = await fetch(
        `https://tajer-platform-api.eyadabdou862.workers.dev/api/public/products/${id}`
      );
      if (!res.ok) throw new Error("Failed to fetch");
  ...
  const handleAddCart = async ({ id }: AddCartParams) => {
    if (isAuthenticated) {
      try {
        setLoadingCart(true);
        const res = await fetch(
          "https://tajer-platform-api.eyadabdou862.workers.dev/api/cart/items",
          {
            method: "POST",
            credentials: "include",
            headers: {
      ...
      >
        <Head>
          ...
          <meta
            name="description"
            content={
              offerData?.description || "Check out this amazing product."
            }
          />
        </Head>
                  ...
                  {isProductOnSale(offerData) && (
                    <Badge className="absolute top-2 right-2 bg-primary z-10">
                      {offerData.discountType === "percentage"
                        ...
                        : `${offerData.discountAmount} ${tid("coins")} ${tP(
                            "offer"
                          )}`}
                    </Badge>
                  )}
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      {language === "ar"
                        ? offerData.factory.name_ar
                        : offerData.factory.name || "Unknown Manufacturer"}
                    </p>
                  </div>
                    ...
                    {isProductOnSale(offerData) ? (
                      <div className="flex gap-2 items-center">
                        <span className="text-3xl font-bold text-primary">
                          {calculateDiscountedPrice(offerData).toFixed(2)}{" "}
                          {tid("coins")}
                        </span>
                  ...
                  <div className="space-y-2">
                    ...
                    {(offerData.unitType === "pack_only" ||
                      offerData.unitType === "piece_or_pack") && (
                      <p className="text-sm">
                    ...
                    <ul className="space-y-1">
                      ...
                      {(offerData.unitType === "pack_only" ||
                        ...
                        <div className="flex gap-2 flex-col">
                          <li className="text-sm text-muted-foreground">
                            {tS("PackPrice")}:{" "}
                            {calculateDiscountedPrice(offerData, true).toFixed(
                              2
                            )}{" "}
                            {tid("coins")}
                            {isProductOnSale(offerData) && (
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    <div className="flex items-center space-x-2 gap-1">
                      <Button
                        ...
                        size="icon"
                        disabled={
                          quantity <= (offerData?.minOrderQuantity || 1) ||
                          loadingCart
                        }
                        onClick={() => {
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          if (
                            !isNaN(value) &&
                            value >= (offerData?.minOrderQuantity || 1)
                          ) {
                            setQuantity(value);
                          }
                        }}
        ...
        <section className="py-12 bg-muted/30 rounded-lg flex flex-col jusitfy-center items-center w-full">
          {loadingProduct ? (
