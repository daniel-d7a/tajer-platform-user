...
export default function SpecialOffers() {
  ...
  const fetchProducts = async (pageNum: number = 1) => {
    try {
      ...
      const res = await fetch(
        `https://tajer-platform-api.eyadabdou862.workers.dev/api/public/products?categoryId=&factoryId=&search=${searchValue}&page=${pageNum}&limit=`
      );
  ...
  const fetchCategories = async () => {
    try {
      const res = await fetch(
        `https://tajer-platform-api.eyadabdou862.workers.dev/api/public/categories?limit=&page=&search=${searchValue}`
      );
      const json = await res.json();
