  ...
  const fetchInvoice = async (page: number = 1, limit: number = 10) => {
    try {
      ...
      const data = await fetch(
        `https://tajer-platform-api.eyadabdou862.workers.dev/api/orders/orders/user?limit=${limit}&page=${page}&status=DELIVERED&from=2025-10-1&to=2025-10-31`,
        { credentials: "include" }
      );
  ...
  return (
    <div className="font-cairo space-y-8 w-full mb-10">
      <div className="bg-card rounded-2xl shadow-sm p-6">
        ...
        <p className="text-muted-foreground mt-2">{t("invoiceSubTitle")}</p>
      </div>
      ...
      <div className="flex justify-between items-center">
        <div className="text-sm text-muted-foreground">
          {to("showing")} {(currentPage - 1) * itemsPerPage + 1} -{" "}
          {Math.min(currentPage * itemsPerPage, totalItems)} {to("of")}{" "}
          {totalItems} {to("invoices")}
        </div>
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="space-y-2">
                <p className="text-lg font-medium text-muted-foreground">
                  {language === "ar"
                    ? "لا يوجد اي طلبات مكتمله حتي الآن!"
