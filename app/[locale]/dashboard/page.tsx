...
import { Copy, DollarSign, ShoppingCart, Truck, Wallet } from "lucide-react";
...
const DashboardPage: React.FC = () => {
  ...
  const fetchOrders = async () => {
    try {
      const data = await fetch(
        "https://tajer-platform-api.eyadabdou862.workers.dev/api/orders/orders/user?limit=&page=&status=PENDING&from=&to=",
        { credentials: "include" }
      );
    ...
    <div className="space-y-8 ">
      ...
      <div className="flex flex-col gap-4">
        <h2 className="text-lg font-bold text-center">
          {td("refferal", { amount: amount })}
        </h2>
        <div className="flex gap-2">
          <Input
            value={`https://tajer-jo.com/register?referredByRepId=${
              userData?.id || 0
            }`}
          />
          <Button
            onClick={() => {
              navigator.clipboard.writeText(
                `https://tajer-jo.com/register?referredByRepId=${
                  userData?.id || 0
                } `
              );
