  ...
  const fetchSuggestions = useCallback(async (query: string) => {
    ...
    try {
      const response = await fetch(
        `https://tajer-platform-api.eyadabdou862.workers.dev/api/public/offers?categoryId=&search=${encodeURIComponent(
          query
        )}&page=1&limit=5`
      );
    ...
    const fetchOffers = async () => {
      try {
        ...
        const res = await fetch(
          `https://tajer-platform-api.eyadabdou862.workers.dev/api/public/offers?categoryId=&search=${
            search || ""
          }&page=${page}&limit=8`
        );
