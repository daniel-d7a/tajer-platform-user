...
import { Boxes, Clock, Factory, Search, Trash2 } from "lucide-react";
  ...
  const fetchSuggestions = useCallback(async (query: string) => {
    ...
    try {
      const response = await fetch(
        `https://tajer-platform-api.eyadabdou862.workers.dev/api/admin/factories?page=1&limit=5&search=${encodeURIComponent(
          query
        )}`
      );
    ...
    const fetchFactory = async () => {
      ...
      try {
        const res = await fetch(
          `https://tajer-platform-api.eyadabdou862.workers.dev/api/admin/factories?page=${page}&limit=20&search=${search}`,
          { credentials: "include" }
        );
