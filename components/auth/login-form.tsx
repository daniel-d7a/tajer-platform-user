  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      const response = await fetch(
        "https://tajer-platform-api.eyadabdou862.workers.dev/api/auth/login",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        if (
          data.message?.includes("user is not active") ||
          data.message?.includes("not active")
        ) {
          throw new Error(
            "حسابك لم يتم تفعيله من قبل الأدمن بعد، يرجى الانتظار"
          );
        } else if (
      ...
      setApiError(
        t("errorPhoneNumberOrPassword") || "حدث خطأ أثناء تسجيل الدخول"
      );
    } finally {
      setIsLoading(false);
    }
