  ...
  const fetchSuggestions = useCallback(async (query: string) => {
    ...
    try {
      const categoriesResponse = await fetch(
        `https://tajer-platform-api.eyadabdou862.workers.dev/api/public/all_categories?search=${encodeURIComponent(
          query
        )}`
      );
      ...
      const productsResponse = await fetch(
        `https://tajer-platform-api.eyadabdou862.workers.dev/api/public/all_products?categoryId=&factoryId=&search=${encodeURIComponent(
          query
        )}`
      );
